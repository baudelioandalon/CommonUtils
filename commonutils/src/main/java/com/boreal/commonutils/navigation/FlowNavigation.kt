...
class FlowNavigation: FrameLayout {
    ...
    fun add(model: Model) {
        val cell = FlowBottomNavigationCell(context)
        cell.apply {
    ...
    private fun anim(cell: FlowBottomNavigationCell, id: Int, enableAnimation: Boolean = true) {
        isAnimating = true
    ...
    fun getCellById(id: Int): FlowBottomNavigationCell? {
    ...
    fun clearCount(id: Int) {
        val model = getModelById(id) ?: return
        val pos = getModelPosition(id)
        model.count = FlowBottomNavigationCell.EMPTY_VALUE
        cells[pos].count = FlowBottomNavigationCell.EMPTY_VALUE
    }

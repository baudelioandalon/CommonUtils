...
public class CUBlurDialogEngine {
     ...
     * Log cat
     */
    private static final String TAG = CUBlurDialogEngine.class.getSimpleName();
     ...
     */
    private CUBlurAsyncTask mBluringTask;
     ...
     */
    public CUBlurDialogEngine(Activity holdingActivity) {
    ...
    public void onResume(boolean retainedInstance) {
        if (mBlurredBackgroundView == null || retainedInstance) {
            if (mHoldingActivity.getWindow().getDecorView().isShown()) {
                mBluringTask = new CUBlurAsyncTask();
     ...
     * {@link CUBlurDialogEngine#DEFAULT_BLUR_DOWN_SCALE_FACTOR}
     * <p/>
     ...
     * {@link CUBlurDialogEngine#DEFAULT_BLUR_RADIUS}
     *
     ...
     * {@link CUBlurDialogEngine#DEFAULT_USE_RENDERSCRIPT}
     * <p/>
    ...
    private void blur(Bitmap bkg, View view) {
        ...
        if (mUseRenderScript) {
            overlay = CURenderScriptBlurHelperKotlin.INSTANCE.doBlur(overlay, mBlurRadius, true, mHoldingActivity);
        } else {
            overlay = CUFastBlurHelperKotlin.INSTANCE.doBlur(overlay, mBlurRadius, true);
        }
        if (mDebugEnable) {
     ...
     */
    private class CUBlurAsyncTask extends AsyncTask<Void, Void, Void> {
